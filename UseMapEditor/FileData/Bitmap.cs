...
namespace UseMapEditor.FileData
{
    public class BMP
    {
        public const int palletstart = 54;

        public static void ImageSizeChange(string inname, string outname, double scale)
        {
            string loadPath = AppDomain.CurrentDomain.BaseDirectory + @"CascData\" + inname.Replace("/", "\\");
            string savePath = AppDomain.CurrentDomain.BaseDirectory + @"CascData\" + outname.Replace("/", "\\");
            // 원본 이미지
            Bitmap sourceImage = new Bitmap(loadPath);
